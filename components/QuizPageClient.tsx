...
/**
 * Página Quiz - Recomendador Inteligente de PCs (parte cliente)
 *
 * Funcionalidad:
 ...
 *
 * Las torres llegan como props desde app/quiz/page.tsx (server component),
 * que las obtiene de la base de datos.
 */
...
export default function QuizPageClient({ allTorres, featuredTorres }: Props) {
  ...
  const findTower = (id: string) => {
    const tower = allTorres.find((p) => p.id === id)!;
    return {
      name: tower.name,
      ...
      <section id="marcas" className="mx-auto max-w-[1180px] px-6 py-16">
        ...
        <div className="flex flex-wrap justify-center gap-6">
          {brands.map((brand) => (
            <div
              key={brand.name}
              className="flex h-32 w-56 items-center justify-center rounded-3xl bg-white px-10 py-8 shadow-lg shadow-black/30 transition duration-300 hover:-translate-y-1 hover:shadow-cyan-400/20"
            >
              <Image
                src={brand.logo}
                alt={brand.name}
